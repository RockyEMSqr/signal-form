...
type Address = { street: string; location: { city: string } };
type Data = {
    addresses: Address[];
...
// Checked by TypeScript; these assertions have no runtime equivalent.
export type PathAssertions = [
    ...
    Assert<Includes<Path<{ nested: { value: string } }>, "nested.value">>,
    Assert<Includes<Path, "any.custom.path">>,
    Assert<Includes<Path<unknown>, "any.custom.path">>
];

type Recursive = { children: Recursive[]; value: string };
...
type Mixed = {
    entries: readonly ({ street: string } | { zip: number } | null)[];
    metadata: any;
