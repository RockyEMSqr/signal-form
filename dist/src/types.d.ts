...
type DotNestedKeysInternal<T, Depth extends number = 3> = Depth extends 0 ? '' : T extends Date ? '' : T extends readonly (infer Element)[] ? `${number}` | `${number}${DotPrefix<DotNestedKeysInternal<Element, Subtract<Depth, 1>>>}` : T extends never ? string : T extends object ? {
