...
export function RichTextAreaInput(p) {
    ...
    return _jsx(_Fragment, { children: _jsxs("div", { class: "form-group orte", children: [p.label && _jsx("label", { for: p.id, class: p.class, children: p.label }), _jsxs("div", { class: "input-group", onClick: onEditorAreaClick, children: [_jsxs("div", { class: "rte-toolbar", children: [Object.keys(actions).map(k => _jsx("button", { type: "button", class: actions[k].on && actions[k].on() ? 'on' : '', onClick: actions[k].exec, children: actions[k].button })), p.toolbarAdditions && _jsx("div", { class: "rte-toolbar-additions", children: p.toolbarAdditions })] }), _jsx(RichTextArea, { ref: editorRef, onInput: onInput, value: value.value, id: p.id }), _jsx("input", { type: "hidden", name: p.name, value: value.value })] })] }) });
}
