...
export function RichTextAreaInput(p) {
    ...
    useEffect(() => {
        var _a;
        (_a = editorRef.current) === null || _a === void 0 ? void 0 : _a.componentDidUpdate();
    }, [p]);
    function exec(c, v) {
        var _a, _b;
        (_a = editorRef.current) === null || _a === void 0 ? void 0 : _a.execCommand(c, false, v);
        (_b = editorRef.current) === null || _b === void 0 ? void 0 : _b.doFocus();
    }
    function qcs(c) {
    ...
    function focusEditor() {
        var _a;
        (_a = editorRef.current) === null || _a === void 0 ? void 0 : _a.doFocus();
    }
    function onEditorAreaClick(e) {
        const target = e.target;
        if (target === null || target === void 0 ? void 0 : target.closest('button, input, select, textarea, a')) {
            return;
        }
        focusEditor();
    }
    ...
    return _jsx(_Fragment, { children: _jsxs("div", { class: "form-group orte", children: [p.label && _jsx("label", { for: p.id, class: p.class, children: p.label }), _jsxs("div", { class: "input-group", onClick: onEditorAreaClick, children: [!p.silent && _jsxs("div", { class: "rte-toolbar", children: [Object.keys(actions).map(k => _jsx("button", { type: "button", class: actions[k].on && actions[k].on() ? 'on' : '', onClick: actions[k].exec, children: actions[k].button })), p.toolbarAdditions && _jsx("div", { class: "rte-toolbar-additions", children: p.toolbarAdditions })] }), _jsx(RichTextArea, { ref: editorRef, onInput: onInput, value: value.value, id: p.id }), _jsx("input", { type: "hidden", name: p.name, value: value.value })] })] }) });
}
